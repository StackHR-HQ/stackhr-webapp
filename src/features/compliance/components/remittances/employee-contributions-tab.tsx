...
import type { RunEmployeeLine } from '../../../payroll/types/payroll-types'

export function EmployeeContributionsTab({ lines, currency }: { lines: RunEmployeeLine[]; currency: string }) {
  return (
      ...
      <table className="w-full text-left text-sm">
        <thead className="bg-surface text-xs uppercase tracking-wide text-muted">
          <tr>
            ...
            <th className="px-4 py-3 font-medium">Total employee contributions</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-line">
          {lines.map((line) => (
            <tr key={line.employeeId} className="bg-canvas">
              ...
              <td className="px-4 py-3 text-ink">{formatAmount(line.pensionEmployee + line.nhfEmployee, currency)}</td>
            </tr>
          ))}
        </tbody>
      </table>
